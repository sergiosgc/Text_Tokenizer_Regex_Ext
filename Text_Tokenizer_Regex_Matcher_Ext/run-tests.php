...
";
	...
	if (array_key_exists('SKIPIF', $section_text)) {
		if (trim($section_text['SKIPIF'])) {
			...
			$output = system_with_timeout("$extra $php $pass_options -n -q $ini_settings $test_skipif", $env);
			if (!$cfg['keep']['skip']) {
				@unlink($test_skipif);
			}
	...
	if (array_key_exists('POST_RAW', $section_text) && !empty($section_text['POST_RAW'])) {
		...
		$cmd = "$php -n$pass_options$ini_settings -f \"$test_file\" 2>&1 < $tmp_post";
		...
		$cmd = "$php -n$pass_options$ini_settings -f \"$test_file\" $args 2>&1";
	}
	...
	if (array_key_exists('CLEAN', $section_text) && (!$no_clean || $cfg['keep']['clean'])) {
		if (trim($section_text['CLEAN'])) {
			...
			if (!$no_clean) {
				...
				$extra = substr(PHP_OS, 0, 3) !== "WIN" ?
					"unset REQUEST_METHOD; unset QUERY_STRING; unset PATH_TRANSLATED; unset SCRIPT_FILENAME; unset REQUEST_METHOD;": "";
				system_with_timeout("$extra $php $pass_options -n -q $clean_params $test_clean", $env);
			}
			if (!$cfg['keep']['clean']) {
				@unlink($test_clean);
			}
